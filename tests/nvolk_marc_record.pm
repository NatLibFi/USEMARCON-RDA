...
sub get_parent_id_from_X73w($) {
    ...
    # Return first ID that looks like a Melinda ID (should we check for multi-MELINDA):
...
sub set_value_sanity_checks($$) {
    # Check that new content looks valid
    my ( $self, $content ) = @_;
    if ( $self->{tag} =~ /^00/ ) { return $content; }
    if ( index($self->{tag}, '9') > -1 && $self->{tag} !~ /^509$/ ) {
	return $content;
    }
    if ( $content !~ /^([0-9 ])/ ) {
	die("Illegal IND1: '$1' in '".kk_marc21_field::fieldToString($content."'"));
    }
    if ( $content !~ /^.([0-9 ])/ ) {
	print "Illegal IND2: '$1'\n";
	if ( $self->{tag} !~ /9/ ) {
	    die($content);
	}
    }
...
sub set_value($$) {
    ...
    &set_value_sanity_checks($self, $content);
    $self->{content} = $content;    
}

sub remove_identical_subfields { # remove duplicate values
    # NB! Not wise enough to handle punctuation.
    my ( $self, $relevant_subfield_codes ) = @_;
    $self->{content} = kk_marc21_field::removeDuplicateSubfields($self->{content}, $relevant_subfield_codes);
}
...
sub content_requires_replication($$) {
    ...
    if ( $self->has_kk_subfield9() ||
	 $self->{content} =~ /\x1F5(FENNI|FIKKA|FI-Vapaa|VIOLA)/ ) {


	# Hacky exception:
	if ( $content && index($content, "\x1F5VIOLA") > -1 &&
	     $self->{content} eq "$content\x1F5VIOLA" ) {
	    return 0;
	}
	...
	if ( $content =~ /\x1F9(FENNI|FIKKA|VIOLA)<KEEP>/ ||
	     $content =~ /\x1F5(FENNI|FIKKA|FI-Vapaa|VIOLA)/ ) {
	    return 1;
	}
    }
    return 0;    
...
sub tag_requires_replication($$) {
	...
	# Requires replication if tag refers to a field transfer:
...
sub sort_relator_terms {
    my ( $self ) = @_;
    $self->{'content'} = kk_marc21_field::sortRelatorTerms($self->{'content'}, $self->{'tag'});
}
...
sub tag2val {
	...
	if ( $tag =~ /^[1678][01]0$/ ) {
	    # Fields with $t (title of a work) come after fields witthout $t:
	    if ( index($content, "\x1Ft") > -1 ) {
		$tag += 0.5;
	    }
	    else {
		# Score based on $e come next
		my $rscore = kk_marc21_field::getMaxRelatorTermScore($content, $tag); # Score is within 0...100s
		
		$tag += 0.1 - $rscore/1000; # 0.1 = 100/1000
	    }
	}
	return $tag;
    }
...
sub containsSubfieldWithValue($$$$) {
	...
	if ( defined($value) ) {
	    ...
	    if ( $fields[$i]->{content} =~ /\x1F\Q${subfield_code}\E/ ) {
		return $fields[$i];
	    }
	}
    }
...
sub get_first_matching_field_index($$) {
    my ( $self, $tag ) = @_;
    if (!defined($tag)) { die(); }
    #if ( !defined($tag) ) { die($self->toString()); }
    my $fieldsP = $self->{fields};
    my @fields = @$fieldsP;
...
sub get_only_matching_field_content($$) {
    my ( $self, $tag ) = @_;
    my $field = $self->get_only_matching_field($tag);
    if ( defined($field) ) {
...
sub get_type_of_record($) {
    my ( $self ) = @_;
    return substr($self->{leader}, 6, 1);
}
...
sub toOAIXML($) {
    my ( $self ) = @_;
    # Header: (for fetched we used shorter version... mere <find-doc> why?)
...
sub replicate_field_to_LOW($$$) {
    ...
    for ( my $i=0; $i <= $#f901; $i++ ) {
	    ...
	    print STDERR "UPDATE 901 to '", kk_marc21_field::fieldToString($content), "'\n";
	    return $f901[$i];
        }
    ...
    my $new_field = $self->add_field('901', $new_content);
    print STDERR " ADD 901\t'", $new_field->toString(), "'\n";
    return $new_field;
}
...
sub is_auth($) {
    ...
    my $type_of_record = $self->get_type_of_record();
    if ( $type_of_record eq 'z' ) {
	die(); # untested
	return 1;
    }
    return 0;
...
sub is_bib($) {
    my ( $self ) = @_;
    my $type_of_record = $self->get_type_of_record();
    if ( index("acdefgijkmoprt", $type_of_record) > -1 ) {
	return 1;
    }
    return 0;
}
...
sub is_holding($) {
    my ( $self ) = @_;
    my $type_of_record = $self->get_type_of_record();
    if ( $type_of_record =~ /^[uvxy]/ ) {
	return 1;
    }
    return 0;
}

sub contains_ownership_metadata($) {
    my ( $self ) = @_;

    my @tags = ( '850', '852', '856', '960', 'LOW' );
    foreach my $tag ( @tags ) {
	if ( $self->has_field($tag) ) { return 1; }
    }
    return 0;
}
...
sub get_form_of_item_position($) {
    my ( $self ) = @_;
    if ( $self->is_mp() || $self->is_vm() ) {
	return 29;
    }
    return 23;
}

sub get_form_of_item($) {
    my ( $self ) = @_;
    my $f008 = $self->get_first_matching_field_content('008');
    if ( defined($f008) ) {
	my $pos = $self->get_form_of_item_position();
	return substr($f008, $pos, 1);
    }
    return undef;
}

sub set_form_of_item($$) {
    my ( $self, $code ) = @_;
    my $f008 = $self->get_first_matching_field('008');
    if ( defined($f008) ) {
	my $pos = $self->get_form_of_item_position();
	if ( $pos == 23 ) {
	    $f008->{content} =~ s/^(.{23})./${1}${code}/;
	    return 1;
	}
	elsif ( $pos == 29 ) {
	    $f008->{content} =~ s/^(.{29})./${1}${code}/;
	    return 1;
	}
    }
    die();
...
sub is_mp($) {
    my ( $self ) = @_;
    # NB! Most computer files are handled differently!
    if ( $self->{leader} =~ /^......[^ef]/ ) { return 0; }
    if ( $self->{leader} =~ /^.......[bis]/ ) {
	my @f006 = $self->get_all_matching_field_contents('006');
	if ( grep(/^s/, @f006) ) {
	    return 1;
	}
	return 0;
...
sub is_cf($) { # CF=Computer File
    my ( $self ) = @_;
    # NB! Most records are not primarily computer files!
    # For those, field 006 should be checked by some *other* function!
    if ( $self->{leader} =~ /^......[^m]/ ) { return 0; }
...
#	}
	if ( $field->{tag} eq 'LOW' ) {
	    if ( $field->{content} =~ /\x1FaFIKKA/ ) {
		$FIKKA = 1;
	    }
	    if ( $field->{content} =~ /\x1FaVIOLA/ ) {
		$VIOLA = 1;
	    }
	}
    }
    if ( $FIKKA ) {
	if ( $VIOLA ) { return 1; }
	if ( $self->containsSubfieldWithValue('042', 'a', 'finbd') ) {
	    return 1;
	}
    }
    return 0;
...
sub is_prepublicationish($) {
    ...
    # Not really sure whether we need this function at all...
    # Currently just alleviate:
    foreach my $field ( @fields ) {
	if ( $field->{tag} eq '856' && $field->{content} =~ /\x1FzKäytettävissä vapaakappalekirjastoissa/ ) {
	    return 0;
	}
    }
...
sub get_unambiguous_host_id($) {
    ...
    # what about 973?
    if ( scalar(@host_item_entry_fields) == 1 && $host_item_entry_fields[0]->{tag} eq '773' ) {
	return $host_item_entry_fields[0]->get_parent_id_from_X73w();
    }
    my %all;
    
    # TODO: all instances of 773
    foreach my $entry ( @host_item_entry_fields ) {
	if ( $entry->{tag} ne '773' ) { return 0; }
	my $curr_id = $entry->get_parent_id_from_X73w();
...
sub is_gradu($) {
    ...
    my @f509 = $self->get_all_matching_field_contents('509');
    @f509 = grep(/gradu/i, @f509);
    if ( scalar(@f509) > 0 ) { return 1; }

    my @f655 = $self->get_all_matching_field_contents('655');
    @f655 = grep(/\x1Fapro gradu -tutkielmat\x1F2slm/i, @f655);
    if ( scalar(@f655) > 0 ) { return 1; }
...
sub is_vaitoskirja($) {
    ...
    my @f502 = $self->get_all_matching_field_contents('502');
    if ( scalar(@f502) > 0 ) { return 1; }

    my @f655 = $self->get_all_matching_field_contents('655');
    @f655 = grep(/\x1Faväitöskirjat\x1F2slm/i, @f655);
    if ( scalar(@f655) > 0 ) { return 1; }
...
sub is_opinnayte($) {
    ...
    my @f50X = $self->get_all_matching_field_contents('50[29]');
    if ( scalar(@f50X) > 0 ) { return 1; }

    my @f655 = $self->get_all_matching_field_contents('655');
    @f655 = grep(/\x1Fa(lisensiaatintutkimukset|opinnäytteet|pro gradu -tutkielmat|väitöskirjat)\x1F2slm/i, @f655);
    if ( scalar(@f655) > 0 ) { return 1; }
...
sub get_unprocessed_publisher($) {
    my ( $self ) = @_;
    my $publisher_field = $self->get_publisher_field();
    if ( defined($publisher_field) && $publisher_field->{content} =~ /\x1Fb([^\x1F]+)/ ) {
	#die($1);
	return $1;
    }
    return undef;
...
sub get_unprocessed_place_of_publication($) {
    ...
    my $field = $self->get_publisher_field();
    if ( defined($field) && $field->{content} =~ /\x1Fa([^\x1F]+)/ ) {
	return $1;
    }
    return undef;
}
...
sub is_japa($) { # JAPA = Musiikkiarkisto
    my ( $self ) = @_;
    if ( $self->is_bib() ) {
	my @f850 = $self->get_all_matching_field_contents('850');
	
	if ( $#f850 > -1 ) {
	    if ( grep(/\x1FaFI-Japa/, @f850) ) {
		return 1;
	    }
	}
    }
    if ( $self->is_holding() ) {
	my @f852 = $self->get_all_matching_field_contents('852');
	foreach my $content ( @f852 ) {
	    $content =~ s/\x1Fz[^\x1F]+//;
	    if ( $content =~ /japa/i ) {
		return 1;
	    }
